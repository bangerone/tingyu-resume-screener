...
2. 列表页 RSC 直接 `await supabase.from('jobs').select(...)`
...
## Acceptance
- [ ] HR 能在 `/admin/jobs/new` 创建一个完整岗位（含 criteria 各种条目）
- [ ] 创建后回列表页能看到，状态默认 `draft`
- [ ] 点编辑能进 detail 页修改保存
- [ ] 切换状态到 `open`，在 Supabase 里查到对应行 `status='open'`
- [ ] 删除有二次确认
- [ ] 未登录访问 `/api/jobs` POST → 401
